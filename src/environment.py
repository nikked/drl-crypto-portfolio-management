...
class TradeEnv:  # pylint: disable=too-many-instance-attributes
    ...
    def step(self, action):  # pylint: disable=too-many-locals
        ...
        index = self.index
        # get Xt from data:
        # data = self.read_tensor(self.data, index)
        done = self.done
        ...
        # the update vector is the vector of the opening price of the day
        # divided by the opening price of the previous day
        update_vector = self.read_update(index)
        ...
        state = (self.read_tensor(self.data, index), w_evol, pf_evol)
